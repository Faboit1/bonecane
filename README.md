...
| Mode | Behaviour |
|------|-----------|
| `legacy` | Bonemeal always grows the plant by 1 block. Has a `double-grow-chance` probability of growing 2 blocks at once. Bonemeal is only consumed when growth happens. |
| `chance` | Bonemeal is consumed on every use. The plant grows only if the random roll beats `grow-chance`. |
